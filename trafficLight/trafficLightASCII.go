...
// A TrafficLightASCII is an abstraction of a real world traffic light
// with the member values being the default lit times
type TrafficLightASCII struct {
...
func (t *TrafficLightASCII) clearScreen() error {
	cmd := exec.Command("clear")
	cmd.Stdout = os.Stdout
	return cmd.Run()
}

// Run runs the traffic light simulation. It will run until interrupted via the
// given Context, or it hits an unrecoverable error
func (t *TrafficLightASCII) Run(c context.Context) {
	...
	for {
		...
		default:
			...
			if err := t.clearScreen(); err != nil {
				fmt.Println(fmt.Sprintf("recieved error : %s, exiting", err))
				break done
			}
...
// the context or context value doesn't evaluate then the defaults passed to the TrafficLightASCII constructor are used
func (t *TrafficLightASCII) retrieveLitFromContextOrDefault(c context.Context) (redLit, yellowLit, greenLit time.Duration) {
	var (
...
// hard coded defaults. If the durations are in spec it will return a *TrafficLightASCII
// or an error if the validation fails
func NewTrafficLight(redLit, yellowLit, greenLit time.Duration) (*TrafficLightASCII, error) {

	if err := validateLitTimes(redLit, yellowLit, greenLit); err != nil {
		return nil, err
	}

	return &TrafficLightASCII{
		redLit:    redLit,
		yellowLit: yellowLit,
		greenLit:  greenLit,
	}, nil
